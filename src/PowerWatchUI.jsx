...
// Fixed survey — comma was missing after first question in original
const SURVEY_QS = [
  ...
  {id:"helped",    type:"choice", q:"Has this platform helped or has the potential to help you plan your day better?",       required:true,  opts:["Yes, it has helped","It has potential","Not sure","No"]},
  ...
  {id:"feedback",  type:"text",   q:"In your own words, how do power outages affect your studies or daily life?",            required:false, placeholder:"e.g. I waste transport money going to check if there is light..."},
];
...
// Rating scale for daily digest
function uptimeRating(hours,T){
  if(hours===null||hours===undefined||isNaN(hours))return{label:"N/A",color:T.t3};
  if(hours===0)return{label:"N/A",color:T.t3};
  if(hours<3) return{label:"POOR",     color:T.red};
  if(hours<6) return{label:"NOT BAD",  color:T.amber};
  if(hours<9) return{label:"GOOD",     color:T.amber};
  if(hours<18)return{label:"EXCELLENT",color:T.green};
  return{label:"PERFECT",color:T.green};
}
...
// Outage duration string (kept for internal use)
...
// Predictor from 7-day data
function getPred(weekly,id){
  if(!weekly||weekly.length<7)return null; // Need full 7 days
  const avg=weekly.reduce((s,d)=>s+(d[id]||0),0)/weekly.length;
  const trend=(weekly[weekly.length-1][id]||0)-(weekly[0][id]||0);
  const best=weekly.reduce((b,d)=>(d[id]||0)>(b[id]||0)?d:b,weekly[0]);
  const worst=weekly.reduce((b,d)=>(d[id]||0)<(b[id]||0)?d:b,weekly[0]);
  return{avg:avg.toFixed(1),trend,best:best.day,worst:worst.day,label:trend>1?"Improving":trend<-1?"Declining":"Stable"};
}

// Get today's uptime from weekly data (last entry = today)
function getTodayUptime(weekly,id){
  if(!weekly||weekly.length===0)return null;
  return weekly[weekly.length-1][id]||0;
}
...
function StatusDot({on,size=10}){
  ...
  return(
    <span style={{width:size,height:size,borderRadius:"50%",background:color,display:"inline-block",flexShrink:0,animation:on?"pulse 2.5s infinite":"none"}}/>
  );
}
...
function StatusPill({on,T}){
  return(
    <span style={{display:"inline-flex",alignItems:"center",gap:"6px",padding:"5px 12px",borderRadius:"20px",background:on?T.greenBg:T.redBg,border:"1px solid "+(on?T.greenBd:T.redBd)}}>
      ...
      <span style={{fontSize:"12px",fontWeight:700,color:on?T.green:T.red,fontFamily:"'DM Sans',sans-serif"}}>{on?"Online":"Offline"}</span>
    </span>
  );
}

// ── Bar chart — with X-axis day labels + Y-axis uptime label ─────────────────
function BarChart({data,color,locId,T}){
  const [hov,setHov]=useState(null);
  if(!data?.length)return<div style={{height:"110px",background:T.raised,borderRadius:"8px"}}/>;
  const vals=data.map(d=>d[locId]||0);
  ...
  return(
    <div>
      <div style={{display:"flex",gap:"6px"}}>
        {/* Y-axis */}
        <div style={{display:"flex",flexDirection:"column",justifyContent:"space-between",alignItems:"flex-end",paddingBottom:"18px",width:"20px"}}>
          {[...yTicks].reverse().map(v=>(
            <span key={v} style={{fontSize:"7px",color:T.t4,fontFamily:"'DM Sans',sans-serif"}}>{v}</span>
          ))}
        </div>
        {/* Bars */}
        <div style={{flex:1}}>
          <div style={{display:"flex",gap:"4px",alignItems:"flex-end",height:"72px"}}>
            {data.map((d,i)=>(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"2px",height:"100%",justifyContent:"flex-end",position:"relative",cursor:"pointer"}} onMouseEnter={()=>setHov(i)} onMouseLeave={()=>setHov(null)}>
                {hov===i&&<div style={{position:"absolute",top:"-22px",background:T.t1,borderRadius:"4px",padding:"2px 6px",fontSize:"9px",color:T.surface,fontWeight:700,whiteSpace:"nowrap",fontFamily:"'DM Sans',sans-serif"}}>{formatUptime(d[locId]||0)}</div>}
                <div style={{width:"100%",borderRadius:"3px 3px 0 0",height:Math.max(((d[locId]||0)/max*68),2)+"px",background:hov===i?color:color+"55",transition:"all 0.15s"}}/>
              </div>
            ))}
          </div>
          {/* X-axis day labels */}
          <div style={{display:"flex",gap:"4px",marginTop:"4px"}}>
            {data.map((d,i)=>(
              <div key={i} style={{flex:1,textAlign:"center"}}>
                <span style={{fontSize:"8px",color:T.t3,fontFamily:"'DM Sans',sans-serif"}}>{(d.day||"").toUpperCase().slice(0,3)}</span>
              </div>
            ))}
          </div>
        </div>
      </div>
      <p style={{fontSize:"8px",color:T.t4,textAlign:"center",marginTop:"3px",fontFamily:"'DM Sans',sans-serif",letterSpacing:"0.5px"}}>POWER UPTIME (HRS)</p>
    </div>
  );
}

// ── Triple line chart — monthly with 3 view options ───────────────────────────
function TripleLineChart({monthly,weekly,color,locId,T}){
  const [hov,setHov]=useState(null);
  ...
  if(view==="monthly"){
    chartData=(monthly||[]).map(m=>({label:MONTH_FULL[m.month_name]||m.month_name||"",value:m[locId]||0}));
    xNote="Monthly total uptime · "+year;
  }else if(view==="weekly"){
    // Approximate weekly average from monthly ÷ 4.33
    chartData=(monthly||[]).map(m=>({label:(MONTH_FULL[m.month_name]||m.month_name||"").slice(0,3),value:m[locId]?parseFloat((m[locId]/4.33).toFixed(1)):0}));
    xNote="Weekly avg uptime · "+year;
  }else{
    chartData=(weekly||[]).map(d=>({label:(d.day||"").toUpperCase().slice(0,3),value:d[locId]||0}));
    xNote="Daily uptime trend · last 7 days";
  }

  if(!chartData.length)return<div style={{height:"80px",background:T.raised,borderRadius:"8px"}}/>;

  const max=Math.max(...chartData.map(d=>d.value),1);
  const W=270,H=60;
  const pts=chartData.map((d,i)=>[
    chartData.length>1?(i/(chartData.length-1))*(W-20)+10:W/2,
    H-(d.value/max)*(H-10)-5
  ]);
  ...
  return(
    <div>
      ...
      <div style={{position:"relative",marginBottom:"12px"}}>
        <select value={view} onChange={e=>setView(e.target.value)} style={{width:"100%",padding:"8px 32px 8px 12px",borderRadius:"8px",background:T.raised,border:"1px solid "+T.border,color:T.t1,fontSize:"12px",fontWeight:600,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",outline:"none",appearance:"none",WebkitAppearance:"none"}}>
          <option value="monthly">Monthly Total Uptime</option>
          <option value="weekly">Weekly Average Uptime</option>
          <option value="daily">Daily Trend (Last 7 days)</option>
        </select>
        <div style={{position:"absolute",right:"10px",top:"50%",transform:"translateY(-50%)",pointerEvents:"none"}}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M6 9l6 6 6-6" stroke={T.t3} strokeWidth="2" strokeLinecap="round"/></svg>
        </div>
      </div>
      {/* Chart */}
      <svg viewBox={"0 0 "+W+" "+(H+20)} style={{width:"100%"}}>
        <defs><linearGradient id={"tg"+locId+view} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.15"/><stop offset="100%" stopColor={color} stopOpacity="0"/></linearGradient></defs>
        <path d={area} fill={"url(#tg"+locId+view+")"}/>
        ...
        {pts.map(([x,y],i)=>(
          <g key={i}>
            ...
            {hov===i&&<g>
              <rect x={x-22} y={y-24} width="44" height="16" rx="4" fill={T.t1}/>
              <text x={x} y={y-13} textAnchor="middle" fontSize="9" fill={T.surface} fontWeight="700" fontFamily="'DM Sans',sans-serif">{formatUptime(chartData[i].value)}</text>
            </g>}
          </g>
        ))}
        {/* X-axis labels — show first, middle, last to avoid crowding */}
        {chartData.map((d,i)=>{
          const show=i===0||i===chartData.length-1||i===Math.floor(chartData.length/2);
          if(!show)return null;
          return <text key={i} x={pts[i][0]} y={H+14} textAnchor="middle" fontSize="7" fill={T.t4} fontFamily="'DM Sans',sans-serif">{d.label.slice(0,3)}</text>;
        })}
      </svg>
      <p style={{fontSize:"8px",color:T.t4,textAlign:"center",marginTop:"2px",fontFamily:"'DM Sans',sans-serif",letterSpacing:"0.3px"}}>{xNote.toUpperCase()}</p>
    </div>
  );
}
...
function PredictorCard({weekly,T}){
  ...
  const p=getPred(weekly,locId); // returns null if < 7 days
  ...
  return(
    <div style={{background:T.card,borderRadius:"16px",padding:"20px",border:"1px solid "+T.border,marginBottom:"12px"}}>
      ...
      {!p?(
        <div style={{background:T.raised,borderRadius:"10px",padding:"14px",border:"1px solid "+T.border,textAlign:"center"}}>
          <p style={{fontSize:"13px",color:T.t3,fontFamily:"'DM Sans',sans-serif",marginBottom:"4px"}}>Not enough data yet.</p>
          <p style={{fontSize:"11px",color:T.t4,fontFamily:"'DM Sans',sans-serif"}}>Robust predictions coming soon — check back after 7 days of sensor data.</p>
        </div>
      ):(
        <>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px",marginBottom:"14px"}}>
            {[["Daily Avg",formatUptime(parseFloat(p.avg)),loc.color],["Best Day",p.best,T.green],["Worst Day",p.worst,T.red]].map(([l,v,c])=>(
              <div key={l} style={{background:T.raised,borderRadius:"10px",padding:"12px 8px",textAlign:"center",border:"1px solid "+T.border}}>
                <p style={{fontFamily:"'Outfit',sans-serif",fontSize:"16px",fontWeight:800,color:c,lineHeight:1,marginBottom:"4px"}}>{v}</p>
          ...
          <div style={{background:T.raised,borderRadius:"10px",padding:"12px 14px",fontSize:"13px",color:T.t2,lineHeight:1.6,border:"1px solid "+T.border,fontFamily:"'DM Sans',sans-serif"}}>
            Expect about <strong style={{color:loc.color}}>{formatUptime(parseFloat(p.avg))}</strong> of power uptime today in {loc.name}. {p.trend>1?"Supply has been improving — a good time to charge your devices.":p.trend<-1?"Supply has been declining — charge your devices as early as possible.":"Supply has been stable this week."}
          </div>
...
function Drawer({loc,live,weekly,monthly,community,onClose,onReport,dark}){
  ...
  // 7-day average — only show if we have full 7 days
  const sevenDayAvg=weekly?.length>=7
    ?parseFloat((weekly.reduce((s,w)=>s+(w[loc.id]||0),0)/7).toFixed(2))
    :null;

  // Today's uptime (last entry in weekly = today's partial or full day)
  const todayUptime=getTodayUptime(weekly,loc.id);

  // Last Reading: time when location was last online (last_updated)
  // Shows HH:MM when offline, N/A when online
  const lastReading=on?"N/A":formatHHMM(d?.last_updated);
    ...
    <div style={{position:"fixed",inset:0,zIndex:300,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      ...
      <div style={{position:"relative",width:"100%",maxWidth:"480px",margin:"0 auto",background:T.surface,borderRadius:"20px 20px 0 0",padding:"0 20px 48px",animation:"slideUp 0.3s cubic-bezier(0.16,1,0.3,1)",maxHeight:"92vh",overflowY:"auto",border:"1px solid "+T.border,borderBottom:"none"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"14px"}}>
          {[
            ["Status",         on?"Online":"Offline",                          on?T.green:T.red],
            ["Power Uptime",   formatUptime(todayUptime),                      T.t1          ],
            ["Last Reading",   lastReading,                                    T.t2          ],
            ["7-Day Average",  sevenDayAvg!==null?formatUptime(sevenDayAvg):"N/A", T.t1     ],
          ].map(([k,v,c])=>(
            <div key={k} style={{background:T.raised,borderRadius:"12px",padding:"14px 16px",border:"1px solid "+T.border}}>
              ...
              <p style={{fontSize:"18px",fontWeight:800,color:c,lineHeight:1,fontFamily:"'Outfit',sans-serif"}}>{v}</p>
            </div>
          ))}
        </div>
        ...
        {/* Triple line chart — monthly with 3 views */}
        <div style={{background:T.raised,borderRadius:"12px",padding:"16px",border:"1px solid "+T.border}}>
          <p style={{fontSize:"11px",fontWeight:600,color:T.t3,textTransform:"uppercase",letterSpacing:"1px",marginBottom:"12px",fontFamily:"'DM Sans',sans-serif"}}>Uptime Trend</p>
          <TripleLineChart monthly={monthly} weekly={weekly} color={loc.color} locId={loc.id} T={T}/>
        </div>
...
function TabBar({tab,setTab,dark}){
  ...
  const TABS=[
    {id:"home",    label:"Home",    icon:(a)=>(<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke={a?accent:T.t3} strokeWidth="2" fill={a?accent+"18":"none"}/><polyline points="9,22 9,12 15,12 15,22" stroke={a?accent:T.t3} strokeWidth="2"/></svg>)},
    {id:"dashboard",label:"Monitor",icon:(a)=>(<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="2" y="3" width="20" height="14" rx="2" stroke={a?accent:T.t3} strokeWidth="2" fill={a?accent+"18":"none"}/><line x1="8" y1="21" x2="16" y2="21" stroke={a?accent:T.t3} strokeWidth="2" strokeLinecap="round"/><line x1="12" y1="17" x2="12" y2="21" stroke={a?accent:T.t3} strokeWidth="2" strokeLinecap="round"/></svg>)},
    {id:"survey",  label:"Survey",  icon:(a)=>(<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M9 11l3 3L22 4" stroke={a?accent:T.t3} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" stroke={a?accent:T.t3} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill={a?accent+"18":"none"}/></svg>)},
  ];
  return(
...
function HomePage({setTab,live,dark}){
  ...
  return(
    <div style={{paddingBottom:"100px",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      {/* Hero — text centred per sketch */}
      <div style={{padding:"80px 24px 40px",background:dark?"linear-gradient(160deg,#0E1B30,#07090F)":T.base,position:"relative",overflow:"hidden",textAlign:"center"}}>
        ...
        <p style={{fontSize:"15px",color:T.t2,lineHeight:1.8,marginBottom:"32px",maxWidth:"340px",margin:"0 auto 32px"}}>
            ...
            <div key={loc.id} onClick={()=>setTab("dashboard")} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"15px 18px",borderBottom:i<LOCS.length-1?"1px solid "+T.border:"none",cursor:"pointer",transition:"background 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background=T.raised} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
                ...
                <div>
                  ...
                  {/* Power Outage: X H XX M | nothing */}
                  <p style={{fontSize:"11px",color:T.t3,marginTop:"1px",fontFamily:"'DM Sans',sans-serif"}}>
                    {!live?"—"
                     :on?"No active outage"
                     :dur?"Power outage · "+dur
                     :"Awaiting sensor"}
                  </p>
                </div>
              </div>
      ...
      {/* Smart Streets In A Nutshell — bigger dots per sketch (50–100% larger) */}
      <div style={{padding:"28px 16px 0"}}>
        <p style={{fontSize:"11px",fontWeight:700,color:T.t3,textTransform:"uppercase",letterSpacing:"2px",marginBottom:"16px",fontFamily:"'DM Sans',sans-serif"}}>Smart Streets In A Nutshell</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",alignItems:"stretch"}}>
          {[
            ...
            <div key={t} style={{background:T.surface,borderRadius:"14px",padding:"18px 16px",border:"1px solid "+T.border,display:"flex",flexDirection:"column",gap:"8px"}}>
              {/* Dot: original was 6px — now 12px (100% bigger) per sketch instruction */}
              <div style={{width:"12px",height:"12px",borderRadius:"50%",background:accent,marginBottom:"2px"}}/>
      ...
      <div style={{margin:"20px 16px 0",padding:"22px",borderRadius:"16px",border:"1px solid "+T.border,background:T.surface}}>
        ...
        <p style={{fontSize:"14px",color:T.t2,lineHeight:1.8,marginBottom:"14px",fontFamily:"'DM Sans',sans-serif"}}>
          Smart Streets is a community-centered project. It utilises sensors to monitor electricity status across UI environs, enabling students and residents to be smarter in their daily activities.
        </p>
        <p style={{fontSize:"14px",color:T.t2,lineHeight:1.8,marginBottom:"16px",fontFamily:"'DM Sans',sans-serif"}}>
          The goal is to provide you with an electricity tracking platform across several locations, accessible to anyone, anywhere — ultimately making you smarter.
        </p>
        {/* Contact section from Image 2 */}
        <div style={{paddingTop:"14px",borderTop:"1px solid "+T.border}}>
          <p style={{fontSize:"12px",fontWeight:600,color:T.t3,textTransform:"uppercase",letterSpacing:"1px",marginBottom:"8px",fontFamily:"'DM Sans',sans-serif"}}>Contact Us</p>
          <p style={{fontSize:"12px",color:T.t2,marginBottom:"4px",fontFamily:"'DM Sans',sans-serif"}}>info@smartstreets.com</p>
          <p style={{fontSize:"12px",color:T.t2,fontFamily:"'DM Sans',sans-serif"}}>enquiry@smartstreets.com</p>
          <p style={{fontSize:"11px",color:T.t4,marginTop:"10px",fontFamily:"'DM Sans',sans-serif"}}>A Student-Led Capstone Initiative under The Fort Institute's FLDC · Cohort 6</p>
        </div>
      </div>
...
function DashboardPage({live,weekly,monthly,community,loading,error,dark,onReport}){
  ...
  return(
    <div style={{padding:"70px 16px 100px",fontFamily:"'DM Sans',sans-serif"}}>
        ...
        {LOCS.map((loc,i)=>{
          ...
          return(
            <div key={loc.id} onClick={()=>!loading&&setSelected(loc)}
              ...
              {/* Today's uptime progress bar */}
              {on&&todayUptime!==null&&(
                <div style={{marginTop:"14px",paddingTop:"14px",borderTop:"1px solid "+T.border}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}>
                    <span style={{fontSize:"11px",color:T.t3,fontFamily:"'DM Sans',sans-serif"}}>Today's power uptime</span>
                    <span style={{fontSize:"11px",fontWeight:700,color:loc.color,fontFamily:"'DM Sans',sans-serif"}}>{formatUptime(todayUptime)}</span>
                  </div>
                  <div style={{height:"4px",background:T.border,borderRadius:"4px",overflow:"hidden"}}>
                    <div style={{width:Math.min((todayUptime/24)*100,100)+"%",height:"100%",background:loc.color,borderRadius:"4px",transition:"width 1s ease"}}/>
                  </div>
                </div>
              )}
            </div>
      ...
      {/* ── DAILY DIGEST (renamed from Yesterday's Summary) ── */}
      {weekly?.length>=2&&(()=>{
        const y=weekly[weekly.length-2]; // yesterday
        ...
        return(
          <div style={{background:T.surface,borderRadius:"16px",padding:"18px",border:"1px solid "+T.border,marginBottom:"12px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
              <div>
                <p style={{fontSize:"11px",fontWeight:600,color:T.t3,textTransform:"uppercase",letterSpacing:"1px",marginBottom:"2px",fontFamily:"'DM Sans',sans-serif"}}>Daily Digest</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px"}}>
              {LOCS.map(loc=>{
                const rawH=y[loc.id];
                // N/A for locations without sensors (null/undefined, not just 0)
                const hasSensor=rawH!==null&&rawH!==undefined;
                const h=hasSensor?parseFloat(rawH.toFixed(2)):null;
                const rating=uptimeRating(h,T);
                return(
                  <div key={loc.id} style={{background:T.raised,borderRadius:"10px",padding:"12px 10px",textAlign:"center",border:"1px solid "+T.border}}>
                    ...
                    <p style={{fontFamily:"'Outfit',sans-serif",fontSize:"18px",fontWeight:800,color:hasSensor?loc.color:T.t4,lineHeight:1,marginBottom:"3px"}}>
                      {hasSensor?formatUptime(h):"N/A"}
                    </p>
                    <p style={{fontSize:"9px",fontWeight:700,color:rating.color||T.t3,textTransform:"uppercase",letterSpacing:"0.5px",fontFamily:"'DM Sans',sans-serif"}}>{rating.label}</p>
                  </div>
                );
              })}
            </div>
      ...
      {/* Power Forecast / Predictor */}
      {weekly&&<PredictorCard weekly={weekly} T={T}/>}
      ...
      <div style={{background:T.surface,borderRadius:"16px",padding:"18px",border:"1px solid "+T.border}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
          ...
          <p style={{fontSize:"11px",color:T.t3,fontFamily:"'DM Sans',sans-serif"}}>avg. uptime per day</p>
        </div>
            ...
            return(
              <div key={loc.id}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:"7px"}}>
                  <span style={{fontSize:"13px",color:T.t2,fontFamily:"'DM Sans',sans-serif"}}>{loc.name}</span>
                  <span style={{fontFamily:"'Outfit',sans-serif",fontSize:"13px",fontWeight:700,color:loc.color}}>{formatUptime(avg)}</span>
                </div>
  ...
  if(done)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"80px 24px",background:T.base,fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{textAlign:"center",maxWidth:"340px"}}>
        ...
        <p style={{fontSize:"14px",color:T.t2,lineHeight:1.7,marginBottom:"24px"}}>Thank you for contributing to this research. Your input helps us build a better grid tracker for students and residents across Agbowo, Orogun and Barika.</p>
